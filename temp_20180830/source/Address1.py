...
class AddressProcess(object):
    ...
    def match_standard(self, group=None, match=None):
        ...
        if level == 0:
            ...
            self.query_standard(standard, match, 0)

        elif level == 2:
            result = self.query_standard(standard, match, 2)
            if not result:
                self.query_standard(standard, match, 4)  # 第3级查找
            # return self.match_city(match)
        elif level == 4:
            result = self.query_standard(standard, match, 4)
            if not result:
                result = self.adjust_changes(match)
                if result:
                    self.query_standard(result)  # 用矫正后的county再去match一次

            # return self.match_county(match)
        # elif level == 6 and group in self.keywords_4:
        elif level == 6:
            result = self.query_standard(standard, match, 6)
        ...
        # elif level == 8:
            ...
            return self.match_village(match, level=8)
        elif level == 10:
    ...
    def match_village(self, village=None, level=None):
        r = area_village[area_village.name.str.find(village) == 0]
        print('village name:', r, )
        if not r.empty:
            self.processed_address.update({self.total_name[level]: r.code,
                                           self.total_name[level+1]: r.name})
        else:
            self.processed_address.update({self.total_name[level]: '',
                                           self.total_name[level+1]: ''})

    def query_standard(self, source, query, level):
        r = source[source.name.str.find(query) == 0]
        print('village name:', r, )
        if not r.empty:
            self.processed_address.update({self.total_name[level]: r.code,
                                           self.total_name[level+1]: '{}{}'.
                                          format(self.processed_address.get(level+1), r.name)})
            return True
        else:
            if 2 <= level <= 8:
                # parent = self.total_name[level-2]
                # code = self.processed_address.get(parent)
                # level, val = [(level, val) for level, val in enumerate(self.processed_address.values() if val is not None ]

                self.processed_address.update({self.total_name[level]: '{}001'.format(self.processed_address.get(level-2)),
                                           self.total_name[level+1]: '{}{}'.format(self.processed_address.get(level+1), query)})
            return False
...
def read_standard_data():
    ...
    global area_village
    area_village = pd.read_csv('area_village.csv', header=1, low_memory=False, dtype=str, names=['parent_code', 'name', 'code'])
    global changes
